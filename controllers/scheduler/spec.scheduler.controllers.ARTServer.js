...
describe('post /schedule/vision/:vision/blueprint/:blueprint',()=>{
                ...
                visionModel.findOne({name:visionSupport.visionAPMChef.name})
                    ...
                    .exec((err,vision)=>{
                        if(err){
                            ...
                            assert.equal(vision.current_projects.length,5);
                        }
                    });
            })
            .catch(err=>{
    ...
    it('shall schedule project to machines, for the machine that does not have enough system resource, it will mark the status to idle /schedule/vision/:vision/blueprint/:blueprint')
    it('shall return error when blueprint is invalid /schedule/vision/:vision/blueprint/:blueprint')
    it('shall return error when vision is invalid')
});